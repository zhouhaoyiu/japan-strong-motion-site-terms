...
The MF2013 regression analysis selected $M_w\geq5.5$ ground-surface records with two horizontal components, at least five triggered stations per event and source distance below 200 km\cite{morikawa2013}. It also truncated data at the magnitude-dependent distance where the Kanno et al. equation predicted PGA below 10 cm s$^{-2}$. Our sensitivity imposes the published magnitude, distance and event-station screens on the primary flatfile sample, then repeats the residual decomposition, event holdout, spatial prediction and spectrum propagation. It contains 53,517 records from 488 earthquakes, including 772 stations with at least 20 records. The Kanno-PGA truncation is not reconstructed; the calculation tests the implemented screens and is not a reconstruction of the original regression sample.
...
\begin{table}[H]
\centering
\caption{MF2013 magnitude, distance and event-station sensitivity}
\scriptsize
\resizebox{\linewidth}{!}{%
\begin{tabular}{rrrrrrr}
\toprule
