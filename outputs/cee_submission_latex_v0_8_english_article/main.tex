...
Regional hazard products average ground motion, yet repeatable station departures can alter local spectra. Applying them requires tests across event samples, ground-motion models and paths. Here we analyse 222,664 K-NET and KiK-net surface records at eight MF2013-compatible RotD100 periods from 0.1 to 5.0 s. At 3.0 s, station terms from disjoint event groups correlate at 0.890; MF2013 and Zhao 2006 fields correlate at 0.770. Site and location variables reduce spatial-block root mean squared error by 12.6\% across 1,628 stations. Source-class fields match the full field, while directional-sector correlations of 0.280--0.886 show path conditioning. Cross-validated multipliers span 0.624--1.338 and change local spectra in both directions. Applying the published MF2013 magnitude, distance and event-station screens gives a 0.959 correlation with the primary field and a 9.9\% spatial-block error reduction at 3.0 s.
...
The primary analysis uses the full domain of the public flatfile. Applying the published MF2013 magnitude, distance and event-station screens leaves 53,517 records from 488 earthquakes. At 3.0 s, station fields estimated from the restricted and primary samples correlate at 0.959. Within the restricted sample, disjoint event groups correlate at 0.895, and the site-and-location model reduces spatial-block error by 9.9\%. The gain is positive from 0.2 to 5.0 s and is $-2.6\%$ at 0.1 s.
...
The screened-sample 3.0 s multipliers span 0.649--1.445 between their 5th and 95th percentiles. On the same 772 stations, the primary prediction changes the surface-spectrum median from 0.089 to 0.091 g, while the restricted prediction changes it to 0.088 g. Local upward and downward changes persist; the direction of the aggregate median depends on the record domain and station set.
