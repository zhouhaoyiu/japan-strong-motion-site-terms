...
Public site and location variables recover part of that average field. The 3.0 s spatial-block gain is 12.6\%, and all five primary folds improve. The stricter K-NET-to-KiK-net experiment separates both stations and earthquakes and achieves a 20.8\% gain; the physical-variable model alone achieves 13.0\%. Reverse transfer is weaker, consistent with the smaller training network. Equal-stratum fields remain predictable in aggregate, with 1--3 s gains of 2.5\%--9.6\%. New regions and source-path distributions still require local records or an explicit path model.
