...
Regional hazard products average ground motion, yet repeatable station departures can alter local spectra. We analyse 222,664 K-NET and KiK-net surface records at eight MF2013-compatible RotD100 periods from 0.1 to 5.0 s. At 3.0 s, disjoint-event station terms correlate at 0.890, and MF2013 and Zhao 2006 fields at 0.770. Across 102,428 paired KiK-net surface--borehole spectra, ratios correlate at 0.988 across event groups, at 0.315 with station terms, and predict held-event terms at 0.290. Site and location variables reduce spatial-block RMSE by 12.6\% across 1,628 stations. Cross-validated multipliers span 0.624--1.338 and change local spectra in both directions. A strict MF2013 regression-domain sensitivity, including the Kanno-PGA cutoff, retains a 0.959 field correlation and a 4.1\% spatial error reduction. The results identify a persistent average-path station field that redistributes matched J-SHIS spectra and requires path conditioning for source-specific use.
...
We evaluate station-field stability under changes in earthquake sample, ground-motion model and path distribution, then apply spatial-block out-of-fold predictions to matched J-SHIS response spectra. Event holdout and a Zhao 2006 calculation test sample and model dependence. A national KiK-net surface--borehole comparison tests whether the residual ordering follows measured paired-sensor amplification. Network, regional, event-influence and path-stratified analyses define the spatial and source-path range of the inferred field. The spectrum calculation quantifies an average-path station adjustment on the published hazard coordinate.
...
The flatfile contains 102,428 KiK-net event--site pairs with positive surface and borehole RotD100 ordinates at each analysed period, covering 699 sites and 1,410 earthquakes. Among 639 sites linked to supported primary station terms, the station term and median surface--borehole ratio at 3.0 s have Pearson and Spearman correlations of 0.315 and 0.415. Removing linear trends with log borehole depth gives a Pearson correlation of 0.327. The Pearson correlation is 0.737 at 0.1 s, 0.522 at 1.0 s and 0.395 at 2.0 s (Fig. \ref{fig:kiknet}; Supplementary Table 4).

The paired ratios are stable across disjoint earthquake groups. At 3.0 s, their mean train--test correlation is 0.988. A line calibrated between training-event ratios and training-event station terms predicts station terms fitted only to held-out events with a mean correlation of 0.290 and a 2.7\% root mean squared error reduction relative to zero. The corresponding correlation and error reduction are 0.471 and 10.0\% at 1.0 s. These held-event results connect the residual station field to a repeatable paired-sensor response while retaining separate events for calibration and evaluation.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.98\linewidth]{figures/figure_kiknet_surface_borehole_validation.pdf}
\caption{KiK-net paired-sensor validation. \textbf{a}, median surface--borehole RotD100 ratio at 3.0 s for 699 paired sites. \textbf{b}, primary MF2013 station terms against paired ratios at supported surface sites; point area scales with paired-record count. \textbf{c}, station-term correspondence and train--test stability of paired ratios across period. \textbf{d}, station-term correlation and root mean squared error reduction when ratios from training earthquakes predict station terms fitted to held-out earthquakes; shading spans the five event folds.}
\label{fig:kiknet}
\end{figure}
...
The site-and-location model improves on the zero baseline at every direct period. Root mean squared error reductions are 10.1\%, 10.3\%, 12.6\% and 12.0\% at 1.0, 2.0, 3.0 and 5.0 s, respectively (Fig. \ref{fig:hazard}b; Supplementary Fig. 2; Supplementary Table 5). A common-feature model that excludes VS20, VS30 and network identity also transfers between K-NET and KiK-net. At 3.0 s it reduces error by 8.7\% when K-NET is held out and by 20.5\% when KiK-net is held out. Fixed macroregion tests are less uniform: the full model improves all five regions by 5.4\% to 28.9\%, whereas the common-feature sensitivity changes from $-5.2\%$ to 27.5\% (Supplementary Fig. 3; Supplementary Table 6). Joint deletion of the ten highest-count events leaves station-term correlations above 0.9989 at every period and changes the 3.0 s field by 0.012 log$_{10}$ units at its record-weighted 95th percentile (Supplementary Table 7).
...
Spatial-block errors provide a separate empirical prediction interval for each held block. At 3.0 s, the nominal 90\% interval covers 89.0\% of stations and 88.8\% after record-count weighting; its median upper-to-lower multiplier span is 3.00 (Supplementary Fig. 6; Supplementary Table 11). These intervals quantify station-model error under the spatial validation design. Source recurrence, ground-motion-model uncertainty and the J-SHIS logic tree remain those of the published hazard product.
...
The national paired-sensor comparison supplies a direct observational check on the station field. Surface--borehole ratios are highly stable across event groups and retain a positive relation to held-event station terms at every period. At 3.0 s, the full-field correlation is 0.315 and the held-event prediction correlation is 0.290, with a 2.7\% error reduction. The weaker long-period correspondence is consistent with a station field that combines local response with unresolved path and backbone effects.
...
Analysis and figure-generation code is supplied in the accompanying peer-review archive. It includes environment specifications, reproduction commands and workflows for the primary and Zhao residual calculations, the MF2013 regression-domain screen, paired-sensor validation, uncertainty propagation, transfer tests, path stratification and balanced-field sensitivity. The same release will be made public at \url{https://github.com/zhouhaoyiu/japan-strong-motion-site-terms} and archived with a DOI before publication.
